...
public class LoginActivity extends AppCompatActivity implements LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button loginButton = (Button) findViewById(R.id.email_register_button);
    ...
    protected void onActivityResult (int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == 1) {
            if (resultCode == RESULT_OK) {
                //Cuando nos registramos correctamente, hacemos login directamente.
                String remail = data.getStringExtra("email");
                String rpw = data.getStringExtra("pw");

                showProgress(true);
                mAuthTask = new UserLoginTask(remail, rpw);
    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            DBConnection dbc = DBConnection.db(getApplicationContext());
            try {
                user_id = dbc.loginUser(mEmail, mPassword);
                res = true;
            }
            catch (LoginException e) {
                if (e.isIncorrectLogin()) {
                    res = false;
                }
            }
            return res;
